...
public:
	...
	void update() //シーンの間は、この中をループする。
	{
		...
		m_data->font(L"新規作成").drawAt(new_map.center, Palette::Black); 
		m_data->font(L"マップを読み込む").drawAt(read_map.center, Palette::Black);
...
public:
	...
	void update() //シーンの間は、この中をループする。
	{
		...
		button.draw(color);
		m_data->font(L"決定").drawAt(button.center, Palette::Black);
		m_data->font(L"マップの名前を入力").draw(0, 0, Palette::Black);
		m_data->font(L"マップ名：").draw(0, 50, Palette::Black);
