...
public:
	...
	void update() //シーンの間は、この中をループする。
	{
		...
		m_data->font(L"地図の属性数を入力してください").draw(); //共通データのfontに従って文字を表示する。
		m_data->font(m_data->cnum).draw(0, 30); //共通データのfontに従って文字を表示する。
...
class INPUT_X : public MyApp::Scene //Xを入力するシーン
{
	public:
		...
		void update() //シーンの間は、この中をループする。
		{
			...
			button.draw(color); //buttonを色＝colorで表示する。
			m_data->font(L"決定").drawAt(button.center,Palette::Black); //buttonの中央にBlackで文字を表示する。
			m_data->font(L"地図の横幅を入力してください").draw(); //共通データのfontに従って文字を表示する。
			m_data->font(m_data->x).draw(0, 30); //共通データのfontに従って文字を表示する。
			Input::GetCharsHelper(text); //textに文字入力を受け取る。
			if(button.leftClicked) //左クリックをした場合
			{
...
public:
	...
	void update() //シーンの間は、この中をループする。
	{
		...
		button.draw(color); //buttonを色＝colorで表示する。
		m_data->font(L"決定").drawAt(button.center, Palette::Black); //buttonの中央にBlackで文字を表示する。
		m_data->font(L"地図の縦幅を入力してください").draw(); //共通データのfontに従って文字を表示する。
		m_data->font(m_data->y).draw(0, 30); //共通データのfontに従って文字を表示する。
		Input::GetCharsHelper(text); //textに文字入力を受け取る。
		if (button.leftClicked) //左クリックをした場合
		{
...
class INPUT_CHECK : public MyApp::Scene //入力が正しいか確認するシーン
{
	public:
		...
		void update() //シーンの間は、この中をループする。
		{
			...
			m_data->font(L"入力は正しいですか？").draw(); //共通データのfontに従って文字を表示する。
			m_data->font(L"属性数",m_data->cnum,L" 横幅：", m_data->x, L" 縦幅：", m_data->y).draw(0, 30); //共通データのfontに従って文字を表示する。
...
class MAKE_MAP : public MyApp::Scene //mapを作るシーン
{
	public:
		...
		RoundRect rect2{ 539 + x_base,429 + y_base,100,50,4 }; //完成ボタンに使う丸角長方形を定義する。
...
class CHECK_MAP : public MyApp::Scene //mapの完成を確認するシーン
{
	public:
		...
		void update()//シーンの間は、この中をループする。
		{
			...
			m_data->font(L"はい").drawAt(Yes.center, Palette::Black); //Yesの中央にBlackで文字を表示する。
			m_data->font(L"いいえ").drawAt(No.center, Palette::Black); //Noの中央にBlackで文字を表示する。
			m_data->font(L"地図は完成しましたか？").draw(); //共通データのfontに従って文字を表示する。
			m_data->font(L"FILE名:", m_data->name).draw(0, 30); //共通データのfontに従って文字を表示する。
...
public:
	...
	void update()//シーンの間は、この中をループする。
	{
		...
		m_data->font(L"はい").drawAt(Yes.center, Palette::Black); //Yesの中央にBlackで文字を表示する。
		m_data->font(L"いいえ").drawAt(No.center, Palette::Black); //Noの中央にBlackで文字を表示する。
		m_data->font(L"別のマップを作りますか？").draw(); //共通データのfontに従って文字を表示する。
		if (Yes.leftClicked) //Yesが左クリックされた場合
		{
