...
public:
	...
	RoundRect read_map{ 100, 160, 200, 50,4 }; //マップを読み込むに使う丸角長方形を定義する。
	...
	void update() //シーンの間は、この中をループする。
	{
		...
		if (new_map.leftClicked)
		{
			...
			changeScene(L"MAKE_MAP"); //シーン"INPUT_X"に移動する。
		}
		else if(read_map.leftClicked)
		{
			changeScene(L"READ_MAP");//シーン"READ_MAP"に移動する。
		}
...
public:
	...
	void update() //シーンの間は、この中をループする。
	{
		...
		m_data->font(m_data->name).draw(180, 50, Palette::Black); //共通データのfontに従って文字を表示する。
		...
		if (button.leftClicked)
		{
			...
			changeScene(L"MAKE_MAP"); //シーン"INPUT_X_READ"に移動する。
		}
...
{
public:
	RoundRect Yes{ 100, 200, 100, 50,4 }; //Yesボタンに使う丸角長方形を定義する。
	RoundRect No{ 230, 200, 100, 50,4 }; //Noボタンに使う丸角長方形を定義する。
	void init()
	{
		Graphics::SetBackground(Palette::White); //背景を白にする
	}
	void update() //シーンの間は、この中をループする。
	{
		...
		m_data->font(L"\nTHE NUMBER OF ATTRIBUTES:", m_data->cnum, L"\n WIDTH：", m_data->x, L"\n HEIGHT：", m_data->y).draw(0, 30, Palette::Black); //共通データのfontに従って文字を表示する。
...
class MAKE_MAP : public MyApp::Scene //mapを作るシーン
{
	...
	public:
		...
		RoundRect rect2{ 640,550,160,50,4 }; //完成ボタンに使う丸角長方形を定義する。
		RoundRect rect3{ 0,550,100,50,4 }; //座標を表示する枠に使う丸角長方形を定義する。
		void init()
		{
	...
	public:
		...
		void update()//シーンの間は、この中をループする。
		{
			...
			if (Yes.leftClicked) //Yesが左クリックされた場合
			{
				...
				make_map(m_data->map, m_data->cnum, m_data->x, m_data->y, L"nmap"+m_data->name.substr(3,2));
...
public:
	...
	void update()//シーンの間は、この中をループする。
	{
		...
		m_data->font(L"EXIT").drawAt(r3.center, Palette::Black);
