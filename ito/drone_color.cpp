...
void Main() {
	...
	while (System::Update()) {
		...
		for (int k = 0; k < (k_d%Z)+1; k++) {
			const Color color_map((k%7) != 0 ? HSV(360/(k%7)) : HSV(0, 0, 100));
			for (int i = 0; i < X; i++) {
				for (int j = 0; j < Y; j++) {
					/*for (int l = k; l < 10 * k; l++) {*/
						if (map[k][i][j] != 0) {
		...
		if ((int)(route[k_d].size()) == 0) {
			...
			camera.pos = (Vec3(route[k_d][i_d].x + 1, k_d+50, route[k_d][i_d].y + 1));
			camera.lookat = Vec3(route[k_d][i_d].x, k_d, route[k_d][i_d].y);
			Graphics3D::SetCamera(camera);
			//Graphics3D::FreeCamera();
			Box(Vec3(route[k_d][i_d].x, (k_d%Z), route[k_d][i_d].y), 1).draw(color_d);
			Cylinder(Vec3(G_X, 0, G_Y), 0.5, Z).draw(Palette::Aqua);
