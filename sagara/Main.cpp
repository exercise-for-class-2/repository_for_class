...
void Drone::avoidance(int move[], int a,int movex, int movey){
		...
		else if (fil[1][2] && move[a] == 3) {		//右に行きたいのに障害物がある!!	ついでにrightはfrontの代わりとして使っているよ！
			if (left && right && back) {			//もうどこにも進めない...世界線を変更しなければ
													//階層の変更
			}
			else if (fil[1][0] || (left && !right)) {			//これ以上左に進めないよぉ.でもまだfrontに行ける
				left = true;
				y += 1;
				return;

			}
			else if (fil[0][1] || (right && !left)) {			//これ以上前に進めないよぉ.でもまだ左に行ける
				right = true;
				x -= 1;
				return;
			}
		...
		else if (fil[1][0] && move[a] == 4) {		//左に行きたいのに障害物がある!!	ついでにleftはfrontの代わりとして使っているよ！
			if (left && right && back) {			//もうどこにも進めない...世界線を変更しなければ
													//階層の変更
			}
