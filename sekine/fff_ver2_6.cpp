...
/*
階層移動で上下移動が可能になった
d.nodes[][]にgoalから近い順にsortされた配列が加わった
sortした配列を使ってset_goal()を更新した
Dijkstra法の精度が上がった

iconとテクスチャのpng画像ファイルは[picture]フォルダにあらかじめいれておくこと
gnuplotで生成されたpng画像ファイルを [image_png]フォルダ内に保存することにした
map0x.dat, nmap0x.dat, input_sg.datはあらかじめ[map]フォルダに入れておくこと
それ以外のすべての生成されたファイルを[output]フォルダ内に保存することにした
...
struct Drone {
	...
	int Zroute[100];			//移動する階層の順番
	int nodes[MAX_Z][MAX_NODE];		//goalから近い順にsortされたnodeが格納された配列
...
bool flag_next = true;					//set_next()内で使う
...
void input_map(std::string file, int map[][Y], bool *flag_map);				//map情報をmap[X][Y]に格納
int set_next(int *z);                                                       //次の階層を決める
int gnuplot_spc(char *file1, char *file2);									//gnuplotを起動して軌跡をpngに描画する								
void input_sg(std::string file);											//input_sg.datを読み込んでstart, goalの座標を代入する
void sort_node(int n, int goal);											//goalから距離が近い順にnodeをsortする


/*-------------------------------------main文-----------------------------------------*/
void Main() {

	const Image image(L"picture/icon.png");
	if (!image) return;
	...
	Camera camera;
	std::string file_sg = "map/input_sg.dat";
	input_sg(file_sg);
	...
	while (l<Z && flag_map) {
		...
		file_map << "map/map" << std::setw(2) << std::setfill('0') << l << ".dat";
	...
	start_drone();
	const Texture texture1(L"picture/wood2.png", TextureDesc::For3D);
	const Texture texture2(L"picture/wall1.png", TextureDesc::For3D);
	const Texture floor(L"picture/wood1.png", TextureDesc::For3D);
	...
	while (System::Update()) {
		...
		if ((int)(route[k_d].size()) == 0) {
			Box(Vec3(xy.x, (k) * 10 + 5, xy.y), 1).draw(texture2);
			i_d = 0;
			k_d++;
			//System::Sleep(0.15s);
			if (k_d == d.i + 1) {
			...
			//LOG(L" move Z k_d=", k_d);
		}
		else {

			camera.pos = (Vec3(route[k_d][i_d].x + 1, k * 10 + 100, route[k_d][i_d].y + 1));
			camera.lookat = Vec3(route[k_d][i_d].x, k, route[k_d][i_d].y);
			Graphics3D::SetCamera(camera);
			Box(Vec3(route[k_d][i_d].x, (k) * 10 + 5, route[k_d][i_d].y), 1).draw(texture2);
			xy = { route[k_d][i_d].x, route[k_d][i_d].y };
			Sphere(Vec3(G_X, k, G_Y), 5).draw(Palette::Aqua);
			font(L"(").draw();  font(route[k_d][i_d].x).draw(31, 0);
			font(L", ").draw(80, 0); font(route[k_d][i_d].y).draw(102, 0);
			font(L", ").draw(151, 0); font(k).draw(173, 0);
			font(L")\n").draw(221, 0);
...
void start_drone() {
	...
	for (int k = 0; k < IZ; k++) {
		...
		nfile_map << "map/nmap" << std::setw(2) << std::setfill('0') << k << ".dat";
		input_map(nfile_map.str(), nmap[k], &flag_gomi);
	}

	std::ofstream os("output/nmap.dat");
	for (int i = 0; i <= X; i++) {
		for (int j = 0; j <= Y; j++) {
			os << nmap[0][i][j] << " ";
		}
...
void Drone::get_map() {
	...
	for (int k = 0; k<IZ; k++) {
		...
		int tmp;
		std::stringstream file_map;
		file_map << "map/map" << std::setw(2) << std::setfill('0') << k << ".dat";
...
void make_dat(int i) {
	...
	sprintf_s(file_all, "output/all%02d.dat", i);
	sprintf_s(file_edges, "output/edges%02d.dat", i);
...
void output_map(int map_buffer[][X][Y]) {
	for (int k = 0; k<IZ; k++) {
		...
		file_map_buffer << "output/map_buffer" << std::setw(2) << std::setfill('0') << k << ".dat";
...
void Drone::Dijkstra() {
	...
	while (x != G_X || y != G_Y || z != G_Z) {
		...
		//fileの名前を自動生成
		std::cout << "z:" << z << '\n';
		file_map << "map/map" << std::setw(2) << std::setfill('0') << z_now << ".dat";
		file_all << "output/all" << std::setw(2) << std::setfill('0') << z_now << ".dat";
		file_edges << "output/edges" << std::setw(2) << std::setfill('0') << z_now << ".dat";
		file_node << "output/node" << std::setw(2) << std::setfill('0') << z_now << ".dat";
		file_dijkstra << "output/dijkstra" << std::setw(2) << std::setfill('0') << i << "_" << std::setw(2) << std::setfill('0') << z_now << ".dat";
		sprintf_s(gnufile_all, "output/all%02d", z_now);
		sprintf_s(gnufile_edges, "output/edges%02d", z_now);
		sprintf_s(gnufile_dijkstra, "output/dijkstra%02d_%02d", i, z_now);
...
void set_goal(int n, int *goal) {
	if (node[d.i][*goal].path != -1) {//goalに到達可能なら
		...
		*/
		int j = 0;
		while ((node[d.i][d.nodes[d.i][j]].path == -1) || map_flag[d.z][node[d.i][d.nodes[d.i][j]].x][node[d.i][d.nodes[d.i][j]].y] == 1) {
			j++;
		}
		*goal = d.nodes[d.i][j];
		int j_push = (n-1) - *goal;
		map_flag_i[d.z].push_back(j_push);
	}
}
...
int gnuplot_spc(char *file1, char *file2) {
	...
	fprintf(gp, "set output 'image_png/gnu_%d_%s.png'\n", i_gnu, file1);
...
void sort_node(int n, int goal) {
	double dis[MAX_NODE];
	for (int j = 0; j < n-1; j++) {
		dis[j] = calc_distance(node[d.i][j].x - node[d.i][goal].x, node[d.i][j].y - node[d.i][goal].y);
	}
	for (int j = 0; j < n - 1; j++) {
		double min = dis[j];
		int min_j = j;
