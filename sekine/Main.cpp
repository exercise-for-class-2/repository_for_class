...
void input_map(std::string file, int map[][Y], bool *flag);					//map情報をmap[X][Y]に格納
...
void Main() {
	...
	while (System::Update()) {
		...
		if ((int)(route[k_d].size()) == 0) {
			...
			camera.pos = (Vec3(route[k_d][i_d].x + 1, k_d%IZ * 10 + 50, route[k_d][i_d].y + 1));
			camera.lookat = Vec3(route[k_d][i_d].x, k_d%IZ, route[k_d][i_d].y);
			Graphics3D::SetCamera(camera);
			//Graphics3D::FreeCamera();
			Box(Vec3(route[k_d][i_d].x, (k_d%IZ) * 10 + 5, route[k_d][i_d].y), 1).draw(color_d);
...
void start_drone() {
	...
	for (int k = 0; k < IZ; k++) {
		...
		input_map(nfile_map.str(), nmap[k], &flag_map);
	}
...
/*-----------------------------------------その他-----------------------------------------------------------*/
void input_map(std::string file, int map[][Y], bool *flag) {
	std::string str;
	int tmp;
	std::ifstream ifile(file.c_str());
	if (!ifile) {
